...
{
    long timeout = longpoll ? (60 * 60) : 30;
    struct data_buffer all_data = {NULL, 0};
    ...
    *rolltime = hi.rolltime;
    ...
    /*It's invalid in the batch case.*/
...
json_t *json_rpc_call2(CURL *curl, const char *url,
                      const char *rpc_req,bool longpoll,
                      struct pool *pool, bool share)
{
    long timeout = longpoll ? (60 * 60) : 30;
    struct data_buffer all_data = {NULL, 0};
    ...
    if (rc)
    {
        applog(LOG_NOTICE, "HTTP request failed: %s", curl_err_str);
        goto err_out;
    }

    if (!all_data.buf)
    {
        applog(LOG_NOTICE, "Empty data received in json_rpc_call.");
        goto err_out;
    }
    ...
    if (!val)
    {
        applog(LOG_NOTICE, "JSON decode failed(%d): %s", err.line, err.text);
    ...
    /*if (!res_val ||(err_val && !json_is_null(err_val)))
    {
        ...
        applog(LOG_NOTICE, "JSON-RPC call failed: %s", s);
...
err_out:
    ...
    curl_easy_reset(curl);
    if (!successful_connect)
        applog(LOG_NOTICE, "Failed to connect in json_rpc_call");
    curl_easy_setopt(curl, CURLOPT_FRESH_CONNECT, 1);
    return NULL;
}
             ...
             void *userp)
{
  ...
  applog(LOG_DEBUG, "%s", text);
  return 0;
}

bool http_post_get_response(CURL *curl, const char *url,
                                const char *request, char *response,
                                uint32_t timeout, bool longpoll, struct pool *pool)
{
    bool bRet = false;
    ...
    if(NULL == curl || NULL == url || NULL == pool
        || NULL == request || NULL == response)
    {
        applog(LOG_ERR, "%s invalid parameter.", __FUNCTION__);
        goto out;
    }else{
        applog(LOG_DEBUG, "%s post %s \n%s", __FUNCTION__, url, request);
    }
    ...
    if(( res == CURLE_OK ) && (res_code == 200 || res_code == 201))
    {
        if(!all_data.buf)
        {
            applog(LOG_ERR, "Empty data received in http response");
            goto out;
        }else{
            bRet = true;
            strncpy(response, all_data.buf, all_data.len);
        }
    }
...
char *bit2String(char *target, char *p, size_t len) {
    char *s = target;
    int i;
    for (i = 0; i < len; i++) {
...
bool hex2bin(unsigned char *p, const char *hexstr, size_t len) {
    ...
    while (*hexstr && len) {
        ...
        if (unlikely((nibble1 < 0) || (nibble2 < 0))) {
            if (p != NULL) {
                applog(LOG_ERR, "hex2bin scan failed: %s", p);
            } else {
...
void print_bin(char *prefix, uint8_t *cmd, size_t len) {
    char *hex_buff = NULL;
...
static bool _valid_hex(char *s, const char *file, const char *func, const int line) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (unlikely(hex2bin_tbl[idx] < 0)) {
            applog(LOG_ERR, "Invalid char 0x%x passed to valid_hex from"
                    IN_FMT_FFL, idx, file, func, line);
            return ret;
        }
    }
    ret = true;
...
static bool _valid_ascii(char *s, const char *file, const char *func, const int line) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (unlikely(idx < 32 || idx > 126)) {
            applog(LOG_ERR, "Invalid char 0x%x passed to valid_ascii from"
                    IN_FMT_FFL, idx, file, func, line);
            return ret;
        }
    }
...
bool fulltest(const unsigned char *hash, const unsigned char *target) {
    ...
    for (i = 28 / 4; i >= 0; i--) {
        ...
        //applog(LOG_NOTICE,"h32tmp:%x,t32tmp:%x\n",h32tmp,t32tmp);
...
bool fulltest64(const unsigned char *hash, const unsigned char *target)
{
    uint32_t *hash32 = (uint32_t *)hash;
    uint32_t *target32 = (uint32_t *)target;
    bool rc = true;
    ...
    if (opt_debug)
    {
        unsigned char hash_swap[8], target_swap[8];
        char *hash_str, *target_str;

        swab64(hash_swap, hash);
        swab64(target_swap, target);
        hash_str = bin2hex(hash_swap, 8);
        target_str = bin2hex(target_swap, 8);
...
static enum send_ret __stratum_send(struct pool *pool, char *s, ssize_t len) {
    ...
    //applog(LOG_NOTICE, "__stratum_send: %s", s);

    return SEND_OK;
}

static enum send_ret __stratum_send_native(struct pool *pool, char *s, ssize_t len) {
    SOCKETTYPE sock = pool->sock;
    ssize_t ssent = 0;
...
bool stratum_send_native(struct pool *pool, char *s, ssize_t len) {
    enum send_ret ret = SEND_INACTIVE;

    mutex_lock(&pool->stratum_lock);
    if (pool->stratum_active)
        ret = __stratum_send_native(pool, s, len);
    ...
    switch (ret) {
        ...
        case SEND_SELECTFAIL:
            applog(LOG_ERR, "Write select failed on pool %d sock", pool->pool_no);
            suspend_stratum(pool);
            break;
        case SEND_SENDFAIL:
            applog(LOG_ERR, "Failed to send in stratum_send");
            suspend_stratum(pool);
            break;
        case SEND_INACTIVE:
...
char *recv_line(struct pool *pool) {
    ...
    if (!strstr(pool->sockbuf, "\n")) {
        ...
        if (!socket_full(pool, DEFAULT_SOCKWAIT)) {
            applog(LOG_ERR, "Timed out waiting for data on socket_full");
            goto out;
        }
        ...
        do {
            ...
            if (n < 0) {
                if (!sock_blocks() || !socket_full(pool, DEFAULT_SOCKWAIT - waited)) {
                    applog(LOG_ERR, "Failed to recv sock in recv_line");
                    suspend_stratum(pool);
                    break;
                }
            } else {
    ...
    if (!tok) {
        applog(LOG_ERR, "Failed to parse a \\n terminated string in recv_line");
        goto out;
    }
    sret = strdup(tok);
    len = strlen(sret);
...
int recv_bin(struct pool *pool, uint8_t *sret, int need_len) {
    ssize_t len = 0;
    int waited = 0;
    uint8_t s[1024];
    ssize_t n = 0;
    ...
    do {
        memset(s, 0, need_len);
        n = recv(pool->sock, s, need_len - len, 0);

        if (!n) {
            applog(LOG_ERR, "Socket closed waiting in recv_bin");
            suspend_stratum(pool);
            break;
        }
        cgtime(&now);
        ...
        if (n < 0) {
            if (!sock_blocks() || !socket_full(pool, DEFAULT_SOCKWAIT - waited)) {
                applog(LOG_ERR, "Failed to recv sock in recv_bin");
                suspend_stratum(pool);
            ...
            memcpy(sret + len, s, n);
            len += n;
            //applog(LOG_ERR, "Socket read back %d, total %d", n, len);
        }
    } while (waited < DEFAULT_SOCKWAIT && (!(len == need_len)));
    return len;
}
...
static bool parse_notify_btc(struct pool *pool, json_t *val) {
    ...
    if (!arr || !json_is_array(arr)) {
        applog(LOG_NOTICE, "parse notify:arr is null \n");
        goto out;
    }
    ...
    snprintf(header, pool->swork.header_len,
             "%s%s%s%s%s%s%s",
             pool->swork.bbversion,//8
             pool->swork.prev_hash,//64
             blank_merkle,//64
             pool->swork.ntime,//8
             pool->swork.nbit,//8
             "00000000",/* nonce 8*/
             workpadding);//96
    //applog(LOG_NOTICE, "header: %s \n",header);
...
static bool parse_notify(struct pool *pool, json_t *val) {
   if (opt_coin_type == COIN_BTC) {
        return parse_notify_btc(pool, val);
    }else {
...
static bool parse_target(struct pool *pool, json_t *val) {
    char *target = json_string_value(json_array_get(val, 0));
    char revtarget[32];
    ...
    cg_wlock(&pool->data_lock);
    //rev(target,64);
    //applog(LOG_NOTICE,"target:%s\n",target);
    hex2bin(pool->target, target, 32);
    rev(pool->target, 32);
    cg_wunlock(&pool->data_lock);

    applog(LOG_NOTICE, "parse_target done\n");
    return true;
}
...
static bool parse_diff(struct pool *pool, json_t *val) {
    ...
    if (old_diff != diff) {
        ...
        applog(LOG_NOTICE, "Pool %d difficulty set to %f", pool->pool_no,
               diff);
...
static bool parse_extranonce(struct pool *pool, json_t *val) {
    ...
    pool->n2size = n2size;
    applog(LOG_INFO, "Pool %d confirmed mining.extranonce.subscribe with extranonce1 %s extran2size %d",
...
static bool parse_reconnect(struct pool *pool, json_t *val) {
    ...
    applog(LOG_DEBUG, "Stratum reconnect requested from pool %d to %s", pool->pool_no, address);
...
bool parse_method(struct pool *pool, char *s)
{
    ...
    applog(LOG_NOTICE,"parse_method:%s",s);
   ...
   if (!strncasecmp(buf, "mining.multi_version", 20))
    ...
    if (!strncasecmp(buf, "mining.notify", 13)) {
        //applog(LOG_NOTICE,"start parse notify");
    ...
    if (!strncasecmp(buf, "mining.set_target", 17)) {
        ret = parse_target(pool, params);
        goto out_decref;
    }
...
extern void srand2(word32 x, word32 y, word32 z);
...
bool auth_stratum(struct pool *pool) {
    ...
    json_error_t err;
    bool ret = false;
		char ccminer_version[20];
	 uint32_t randomv=0x233;
	...
	applog(LOG_ERR, "auth recv: %s", sret);
    ...
    if (!res_val || json_is_false(res_val) || (err_val && !json_is_null(err_val))) {
        ...
        if (err_val)
            ...
            ss = strdup("(unknown reason)");
        applog(LOG_ERR, "pool %d JSON stratum auth failed: %s", pool->pool_no, ss);
        free(ss);
    ...
    ret = true;
    applog(LOG_NOTICE, "Stratum authorisation success for pool %d", pool->pool_no);
    pool->probed = true;
    successful_connect = true;
    if (opt_suggest_diff) {
        ...
        if (connect(sockd, p->ai_addr, p->ai_addrlen) == -1) {
            ...
            if (selret > 0 && FD_ISSET(sockd, &rw)) {
                ...
                if (!n && !err)
                {
                    ...
                    applog(LOG_DEBUG, "Succeeded delayed connect ip: %s:%u", ipAddr, ipv4->sin_port);
                    block_socket(sockd);
                    break;
                }
            }
...
static char *get_sessionid(json_t *val) {
    ...
    if (arrsize != 0) {
        char *notify = __json_array_string(arr_val, 0);
        if (notify != NULL) {
...
void suspend_stratum(struct pool *pool) {
    applog(LOG_ERR, "Closing socket for stratum pool %d", pool->pool_no);
...
bool initiate_stratum(struct pool *pool) {
    ...
    if (recvd) {
        ...
        sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": []}", swork_id++);
			...
			randomv = rand();
			sprintf(ccminer_version,"2.%d.%d",randomv&0x7,(randomv>>0x4)&0x7);
        if (pool->sessionid){
            //sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""PACKAGE"/"VERSION"\", \"%s\"]}", swork_id++, pool->sessionid);
            sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\"%s/%s\", \"%s\"]}", swork_id++, g_version_miner[randomv&0x7], ccminer_version,pool->sessionid);
        }
        else{
            //sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""PACKAGE"/"VERSION"\"]}", swork_id++);
            sprintf(s, "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\"%s/%s\"]}", swork_id++,g_version_miner[randomv&0x7], ccminer_version);
		 }
    }
    ...
    applog(LOG_NOTICE, "Subscribe RECVD 2--- : %s", sret);
    ...
    if (!res_val || json_is_null(res_val) ||
        ...
        applog(LOG_NOTICE, "JSON-RPC decode failed: %s", ss);
        ...
        if (!sessionid) {
            applog(LOG_NOTICE, "Failed to get sessionid in initiate_stratum");
            goto use_address;
        }
...
out:
    if (ret) {
        ...
        applog(LOG_NOTICE, "initiate_stratum stratum is active.");
        pool->sdiff = 1;
        if (opt_protocol) {
...
bool restart_stratum(struct pool *pool) {
    ...
    if (pool->stratum_active) {
        applog(LOG_DEBUG, "restart_stratum now !!!!");
        suspend_stratum(pool);
    }
...
void cg_logwork(struct work *work, unsigned char *nonce_bin, bool ok) {
    if (opt_logwork_path) {
        ...
        if (strcmp(opt_logwork_path, "screen") == 0) {
            ...
            applog(LOG_ERR, szmsg);
            if (g_logwork_file) {
                sprintf(szmsg, "%s %08x work %s midstate %s data %s nonce %s hash %s diff %"PRId64"", ok ? "o" : "x",
                        work->id, szworkdata, szmidstate, szdata, sznonce5, szhash, worksharediff);
                ...
                if (ok) {
                    ...
                    if (opt_logwork_diff) {
                        ...
                        applog(LOG_DEBUG, "work diff %"
                                PRId64
                                " diffnum %d", worksharediff, diffnum);
                        sprintf(szmsg, "midstate %s data %s nonce %s hash %s", szmidstate, szdata, sznonce4, szhash);
