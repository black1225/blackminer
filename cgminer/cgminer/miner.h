...
static inline void swab64(void *dest_p, const void *src_p)
{
    uint32_t *dest = dest_p;
    const uint32_t *src = src_p;
...
static inline int
_mutex_trylock(pthread_mutex_t *lock, __maybe_unused const char *file, __maybe_unused const char *func,
...
static inline int _wr_trylock(pthread_rwlock_t *lock, __maybe_unused const char *file, __maybe_unused const char *func,
                              __maybe_unused const int line) {
    TRYLOCK(lock, file, func, line);
...
extern char *bit2String(char *t, char *p, size_t len);
...
extern char *g_mac_addr;
extern unsigned int g_random_32bit;
...
extern char displayed_av_hash_rate[16];
extern char displayed_av_hash_rate2[16];
...
struct work {
    ...
    unsigned char midstate[200];
    unsigned char target[32];
    unsigned char hash[32];
    unsigned char pre_nonce[32];
...
extern struct work *
__find_work_bymidstate(struct work *que, char *midstate, size_t midstatelen, char *data, int offset, size_t datalen);

extern struct work *
find_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset,
                            size_t datalen);

extern struct work *
clone_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset,
                             size_t datalen);
...
extern struct work *
take_queued_work_bymidstate(struct cgpu_info *cgpu, char *midstate, size_t midstatelen, char *data, int offset,
                            size_t datalen);
...
#define A3_MINER_ASIC_NUM_EACH_CHAIN            1
#elif defined(USE_BLACKMINER_F3)
#define A3_MINER_ASIC_NUM_EACH_CHAIN            2
#else
#define A3_MINER_ASIC_NUM_EACH_CHAIN            6
#endif
#define ASIC_NUM_EACH_CHAIN                     A3_MINER_ASIC_NUM_EACH_CHAIN

#define COIN_BTC 0

#define USE_POOL_TIMESTAMP
#define VERIBLOCK_POSITIVE_MAX_OFFSET 140
#define VERIBLOCK_NEGATIVE_MAX_OFFSET -2000

#define VERIBLOCK_MAINNET_POSITIVE_MAX_OFFSET 10
